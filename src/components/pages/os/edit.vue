    ...
    <b-card border-variant="white">
      <b-row>
        ...
        <b-col cols="4" class="text-right">
          ...
          <b-button @click.prevent="downloadPdf()" :disabled="validation || (os.selected_services && os.selected_services.length === 0)" variant="primary">Visualizar PDF</b-button>
        </b-col>
      </b-row>
      <b-card-body>
        <b-row>
...
export default {
  ...
  created () {
    this.$store.dispatch("getOsById", this.$route.params.id).then(resp => {
      this.os = resp.data
      this.calcOsValue()
      this.load = true
    });
  ...
  methods: {
    ...
    downloadPdf () {
      this.spinner = true;
      this.$store.dispatch("editOs", this.os).then(() => {
        this.$store.dispatch('osPdf', { id: this.$route.params.id }).then(response => {
    ...
    submitOs () {
      this.spinner = true;
      this.$store.dispatch("editOs", this.os).then(
        this.$bvToast.toast('Proposta editada com sucesso', {
          autoHideDelay: 4000,
          variant: 'success',
          appendToast: true,
          noCloseButton: true,
