<template>
  <div class="vdatetime">
    ...
    <input class="vdatetime-input" :class="inputClass" :style="inputStyle" :id="inputId" type="text"
      placeholder="dd/mm/aaaa" :value="inputValue" v-bind="$attrs" v-on="$listeners" @click="open" @focus="open">
...
export default {
  ...
  // watch: {
  //   value () {
  //     this.datetime = datetimeFromISO(oldVal)
  //   }
  // },
  ...
  computed: {
    inputValue() {
      let format = ''
      // console.log(DateTime.fromISO(this.datetime).setZone(this.zone).toFormat(format))
      if (!format) {
        switch (this.type) {
          case 'date':
            format = DateTime.day
